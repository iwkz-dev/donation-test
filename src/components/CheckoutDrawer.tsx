...
import type { PaypalCheckoutItem } from "@/types/api";
...
export function CheckoutDrawer() {
    ...
    const { config, configLoading, loading, calculatePaypalFee, submitPaypalCheckout } =
        usePayment();
    ...
    const isSubpackageMode = Boolean(
        selectedPackage && (selectedPackage.donationItems.length > 1 || isFixedSingleItem)
    );
    ...
    const activeUniqueCodes = isSubpackageMode
        ? cartItems
            .filter((item) => item.quantity > 0)
            .map((item) => item.uniqueCode)
            .join(", ")
        : singleItem?.uniqueCode ?? "";
    ...
    return (
        <>
            {/* Backdrop */}
            <div
                className={`fixed inset-0 z-40 bg-black/50 backdrop-blur-sm transition-opacity duration-300 ${drawerOpen ? "opacity-100" : "pointer-events-none opacity-0"
                    }`}
                onClick={closeDrawer}
            />
            ...
            <div
                ref={drawerRef}
                className={`fixed inset-x-0 bottom-0 z-50 mx-auto max-w-md transform transition-transform duration-300 ease-out ${drawerOpen ? "translate-y-0" : "translate-y-full"
                    }`}
            >
                <div className="max-h-[90vh] overflow-y-auto rounded-t-3xl bg-white shadow-2xl">
                    ...
                    <div className="px-6 pb-8">
                        ...
                        {/* Total */}
                        {totalPrice > 0 && (
                            <div className="mb-6 rounded-2xl bg-gradient-to-r from-emerald-50 to-teal-50 p-4">
                        ...
                        {paymentTab === "bank" && config && (
                            <div className="mb-6 animate-fade-in rounded-2xl border border-gray-100 bg-gray-50/50 p-5">
                                ...
                                {[
                                    { label: "Bank", value: config.postbank.bankName },
                                    { label: "IBAN", value: config.postbank.iban },
                                    { label: "BIC", value: config.postbank.bic },
                                    { label: "Verwendungszweck", value: activeUniqueCodes || "Donasi" },
                                ].map(({ label, value }) => (
                                    <div
                                        key={label}
                                                ...
                                                <div className="flex justify-between text-sm">
                                                    <span className="text-gray-500">
                                                        ...
                                                        <span className="text-xs text-gray-400">
                                                            ({(config.paypal.percentageFee / 100).toFixed(2)}% + {formatCurrency(config.paypal.fixFee / 100)})
                                                        </span>
                                                    </span>
