...
export function DonationCard({ donationPackage: pkg, index }: DonationCardProps) {
    const openDrawer = useDonationStore((s) => s.openDrawer);

    const totalDonation = pkg.donationItems.reduce((sum, item) => sum + (item.total_donation ?? 0), 0);
    const totalOrder = pkg.donationItems.reduce((sum, item) => sum + (item.total_order ?? 0), 0);
    const targetDonation = pkg.donationItems.reduce((sum, item) => sum + (item.targetDonation ?? 0), 0);
    ...
    return (
        <button
            ...
            className="group relative flex flex-col w-full text-left rounded-[2rem] bg-white border border-slate-100 p-6 shadow-sm transition-all duration-300 hover:shadow-2xl hover:shadow-emerald-900/5 hover:-translate-y-1 active:scale-[0.98] overflow-hidden"
                ...
                <div className="flex items-start justify-between">
                    {pkg.image?.url ? (
                        <div className="h-14 w-14 rounded-2xl overflow-hidden shrink-0 shadow-sm border border-slate-100 transition-transform duration-300 group-hover:scale-105">
                            <img src={pkg.image.url} alt={pkg.title} className="h-full w-full object-cover" />
                        </div>
                    ) : (
                        <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-teal-50 to-emerald-50 text-emerald-600 transition-transform duration-300 group-hover:scale-105 group-hover:from-teal-100 group-hover:to-emerald-100 shrink-0">
                    ...
                    {pkg.donationItems.length > 1 && (
                        <div className="flex h-7 px-3 items-center justify-center rounded-full bg-gradient-to-r from-amber-400 to-orange-400 text-xs font-bold text-white shadow-md">
            ...
            <div className="mt-auto pt-4 flex flex-col gap-5 border-t border-slate-50">
                ...
                {hasTarget ? (
                    ...
                    <div className="h-[34px] flex items-center">
                        <span className="text-xs font-medium text-slate-400">Donasi Terbuka</span>
                    </div>
                )}
                ...
                <div className="flex items-center justify-between">
                    <div className="flex flex-col">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-0.5">Terkumpul</span>
                    ...
                    <div className="flex flex-col items-end">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-0.5">Donatur</span>
